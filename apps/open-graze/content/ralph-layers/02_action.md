...
- [x] 다음 핸드오프 Action: Stage 2 항목별 상태(대기/진행/완료)를 `flow.actionItems` 확장 필드로 받을 수 있도록 표시 스키마 초안을 정의한다.
- [ ] 다음 핸드오프 Action: Stage 2 UI가 `flow.actionItemDrafts[].status`를 배지(대기/진행/완료)로 시각화하고 수동 상태 변경 API 연동 포인트를 문서화한다.
