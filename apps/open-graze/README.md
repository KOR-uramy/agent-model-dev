...
선택 쿼리 **`role`**·**`sessionId`**는 `GET /api/ralph/events`와 **같은 규칙**입니다(`role`은 네 가지 허용값만, 그 외 비어 있지 않은 값은 **400**; `sessionId`는 SQLite `payload`의 `sessionId`와 **문자열 정확 일치**).

| 파라미터 | 필수 | 의미 |
|----------|------|------|
| `from` | 예 | 구간 시작(ISO 8601) |
| `to` | 예 | 구간 끝(ISO 8601) |
| `role` | 아니오 | `planning` \| `design` \| `implementation` \| `test` — `detail.role`과 같은 행만 |
| `sessionId` | 아니오 | 해당 세션 ID와 정확히 같은 행만 |
| `limit` | 아니오 | 반환 상한(기본 10000, 최대 **10000**) |
