...
- `DATABASE_URL="file:./prisma/dev.db"` (기본값 유지 가능)
...
- **개발 전제(현재 스키마)**: `prisma/schema.prisma`의 `datasource db`는 `provider = "sqlite"`이며, 로컬은 `DATABASE_URL="file:./prisma/dev.db"` 형태의 **파일 DB**를 씁니다.
