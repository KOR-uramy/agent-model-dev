...
동기화된 SQLite 타임라인만 읽습니다. 응답 본문은 **객체**이며, `events`가 `WorkspaceFeedEvent` 배열(대시보드 `GET /api/ralph/events`와 동일 형태)이고, SQLite `LIMIT`에 도달하면 최상위 **`truncated`: `true`**(그렇지 않으면 `false`), **`returnedCount`**는 `events.length`입니다.

**쿼리 파라미터**(`GET /api/ralph/events`와 동일한 **`role`** 허용값·400 규칙, **`sessionId`** 는 페이로드 **`sessionId` 문자열과 정확 일치**):

| 파라미터 | 필수 | 의미 |
|----------|------|------|
| `from` | 예 | 구간 시작(ISO 8601). `to`보다 이후이면 400. |
| `to` | 예 | 구간 끝(ISO 8601). |
| `role` | 아니오 | `planning` \| `design` \| `implementation` \| `test` — `detail.role`과 같은 행만. 생략·공백만이면 필터 없음. 그 외 비어 있지 않은 값은 **400**(`GET /api/ralph/events`와 동일 메시지). |
| `sessionId` | 아니오 | 해당 **세션 ID**와 정확히 일치하는 이벤트만. 공백만이면 필터 없음(쿼리 키만 있는 경우와 동일). |
| `limit` | 아니오 | SQLite에서 가져올 **최대 행 수**. 기본 `10000`, **상한 `10000`**(`TIMELINE_RANGE_MAX_ROWS`). |
- **잘림 확인(해당 구간에 이벤트가 1건 이상 있을 때 `limit=1`이면 상한에 닿음)**: `curl -sS 'http://localhost:3000/api/ralph/events/range?from=1970-01-01T00:00:00Z&to=2099-12-31T23:59:59Z&limit=1' | node -p "JSON.parse(require('fs').readFileSync(0,'utf8')).truncated"` → 기대값 `true`.
- **파일 저장 예**:
...
```bash
printf 'all=%s role=planning=%s\n' "$(curl -sS 'http://localhost:3000/api/ralph/events/range?from=2026-05-01T00:00:00Z&to=2026-05-03T23:59:59Z' | node -p "JSON.parse(require('fs').readFileSync(0,'utf-8')).events.length")" "$(curl -sS 'http://localhost:3000/api/ralph/events/range?from=2026-05-01T00:00:00Z&to=2026-05-03T23:59:59Z&role=planning' | node -p "JSON.parse(require('fs').readFileSync(0,'utf-8')).events.length")"
```

`role`·`sessionId`를 **같은 요청**에 넣는 **한 줄** 예(`SESSION`을 실제 ID로 바꿉니다):

```bash
curl -sS 'http://localhost:3000/api/ralph/events/range?from=2026-05-01T00:00:00Z&to=2026-05-03T23:59:59Z&role=planning&sessionId=SESSION' | node -p "JSON.parse(require('fs').readFileSync(0,'utf-8')).events.length"
```
