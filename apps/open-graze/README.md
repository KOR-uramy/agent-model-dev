...
```bash
printf 'all=%s role=planning=%s\n' "$(curl -sS 'http://localhost:3000/api/ralph/events/range?from=2026-05-01T00:00:00Z&to=2026-05-03T23:59:59Z' | node -p "JSON.parse(require('fs').readFileSync(0,'utf-8')).length")" "$(curl -sS 'http://localhost:3000/api/ralph/events/range?from=2026-05-01T00:00:00Z&to=2026-05-03T23:59:59Z&role=planning' | node -p "JSON.parse(require('fs').readFileSync(0,'utf-8')).length")"
```

`role`·`sessionId`를 **같은 요청**에 넣는 **한 줄** 예(`SESSION`을 실제 ID로 바꿉니다):

```bash
curl -sS 'http://localhost:3000/api/ralph/events/range?from=2026-05-01T00:00:00Z&to=2026-05-03T23:59:59Z&role=planning&sessionId=SESSION' | node -p "JSON.parse(require('fs').readFileSync(0,'utf-8')).length"
```
