...
`GET /api/ralph/events`는 선택 쿼리 **`role`**(`planning` \| `design` \| `implementation` \| `test`)로 **`detail.role`이 같은 이벤트만** 최근 `tail`건까지 반환합니다. 그 외 값은 **400**입니다. 선택 쿼리 **`source`**(`ralph` \| `application`)는 타임라인 한 줄의 **`source`** 와 같은 집합만 허용하며, **빈 문자열 `source=`를 포함한** 그 외 값은 **400**입니다. 아래 한 줄로 필터 유·무 응답의 **`events` 배열 길이**를 바로 비교할 수 있습니다(개발 서버·DB에 타임라인이 있을 때).
...
`source`로 **전체 대비 한 출처만** 건수를 비교합니다.

```bash
printf 'all=%s source=application=%s\n' "$(curl -sS 'http://localhost:3000/api/ralph/events?tail=800' | node -p "JSON.parse(require('fs').readFileSync(0,'utf-8')).events.length")" "$(curl -sS 'http://localhost:3000/api/ralph/events?tail=800&source=application' | node -p "JSON.parse(require('fs').readFileSync(0,'utf-8')).events.length")"
```
...
- **쿼리**: `from`, `to` — 필수, ISO 8601(예: `2026-05-01T00:00:00Z`, `2026-05-03T23:59:59.999Z`). `limit` — 선택, 기본 10000, 상한 10000. 선택 **`source`**(`ralph` \| `application`) — 페이로드 `source`와 동일 집합, **빈 문자열 `source=`를 포함한** 잘못된 값은 **400**.
...
`source`로 구간 내 **전체 대비 한 출처만** `returnedCount`를 비교합니다.

```bash
printf 'all=%s source=ralph=%s\n' "$(curl -sS 'http://localhost:3000/api/ralph/events/range?from=2026-05-01T00:00:00Z&to=2026-05-03T23:59:59Z' | node -p "JSON.parse(require('fs').readFileSync(0,'utf-8')).returnedCount")" "$(curl -sS 'http://localhost:3000/api/ralph/events/range?from=2026-05-01T00:00:00Z&to=2026-05-03T23:59:59Z&source=ralph' | node -p "JSON.parse(require('fs').readFileSync(0,'utf-8')).returnedCount")"
```
