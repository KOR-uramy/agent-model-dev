...
#
# Runs Codex CLI locally with JSON event logging.
# Codex 복제본은 Cursor용 resume/rotation 대신 각 이터를 독립 실행한다.
#
...
#   ./ralph-loop.sh -n 50 -m gpt-5.4             # Custom iterations and model
...
#   -m, --model MODEL      Model to use (default: gpt-5.2; override with RALPH_MODEL)
...
Environment:
  RALPH_MODEL            Override default model (same as -m; Codex copy default: gpt-5.2)
...
main() {
  ...
  local requested_workspace="$WORKSPACE"
  local task_root=""
  if task_root="$(ralph_find_task_root "$WORKSPACE")"; then
    WORKSPACE="$task_root"
  fi
  ...
  if [[ "$requested_workspace" != "$WORKSPACE" ]]; then
    echo "Requested: $requested_workspace"
  fi
  echo "Task:      $task_file"
  echo ""
  ...
  if [[ "$SKIP_CONFIRM" != "true" ]]; then
    echo "This will run Codex CLI locally to work on this task."
