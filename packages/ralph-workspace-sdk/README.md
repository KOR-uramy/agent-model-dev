...
Ralph `stream-parser`의 **`.ralph/events.jsonl`**과, 앱이 남기는 **`.ralph/workspace-telemetry.jsonl`**을 합쳐 읽습니다. Next.js(**OpenGraze** = **Workspace Platform**, 코드는 `apps/open-graze`) API 한 줄·**작업 로거**로 같은 앱에 붙일 수 있습니다.
