...
3. **반복** — `ralph-loop.sh`는 기본 **역할 파이프**(기획→디자인→구현→테스트)로 이터를 돌린다. 각 이터는 직전 역할의 산출물(git·`.ralph/progress.md`)을 **감시**한 뒤 본 역할만 수행한다. 한 사r이클 끝에는 루트 `npm run build` 등 검증·커밋·기준 `[x]`·`.ralph/progress.md` 요약이 이어진다. 단일 프롬프트만 쓰려면 `RALPH_ROLE_MODE=mono`를 본다.
...
**UI/정보구조 원칙** — 가능한 한 **모든 기능은 먼저 한 페이지에 모아** 실제 작업 흐름을 만든다. 다만 **서비스 사용자에게 의미 있는 기능만** 기본 UI 단계에서 노출하고, 내부 디버그성·실험성 기능은 기본 화면에서 숨기거나 약하게 둔다. 한 페이지에 서비스용 기능이 많아져 **화면이 지저분해지기 시작하면**, 그때서야 **카테고리화**해서 탭·추가 페이지·보조 패널로 분리한다. 즉, **조기 분산보다 한곳 집중**, **무분별한 노출보다 의미 있는 기능 선별**, **과밀 시에만 구조 분해**를 기본 정책으로 둔다.
...
- **UI/정보구조 정책** — 기본은 **한 페이지에 가능한 기능을 먼저 모아** 실제 사용 흐름을 검증한다. 다만 서비스 사용자에게 **의미 있는 기능만** 기본 화면에 노출하고, 디버그·실험 기능은 숨기거나 약하게 둔다. 이후 한 페이지의 서비스용 기능이 많아져 혼잡해질 때만 **카테고리화**해서 탭·추가 페이지·보조 패널로 분리한다.
...
- [x] **URL·재현 — `from`·`to` 쿼리 동기화** — `/`에서 사용자가 고른 **절대 시간 구간**이 **`?from=`·`?to=`**(ISO 8601; timezone 포함)와 **양방향** 동기화된다. 규칙을 고정한다: (1) `from`/`to` 중 **하나만** 있으면 **둘 다 무효**(UI는 주소줄에서 키 제거, API는 400), (2) 둘 다 있을 때 UI는 값을 파싱해 **정규화된 ISO(UTC `Z`)** 로 주소줄을 다시 써 **동일 URL 재현**이 가능하게 한다, (3) 이 구간은 `role`·`sessionId`·`source`와 **AND 조합**이며 `tail`은 “모드”가 아니라 **구간 내 최대 행 수 상한**으로만 동작한다. **검증 힌트(테스트 역할)**: 같은 URL을 새 탭/시크릿 창에 붙여넣었을 때 주소창의 `from`·`to`가 **정규화된 `Z`** 로 유지되고, 필터 UI와 응답 행이 원본 탭과 일치한다. `apps/open-graze/README.md`에 **`?role=`·`?sessionId=`·`?from=`·`?to=` 를 모두 고정한 복사 가능한 예시 URL 한 줄**이 있다.
- [x] **API·UI — `source` 필터** — `GET /api/ralph/events`와 `GET /api/ralph/events/range`에 선택 쿼리 **`source`**가 있으며, 허용값 집합은 타임라인 페이로드의 `source`와 **동일**하게 “한 곳”에서 고정한다(예: `ralph` \| `application`). **계약(디자인 역할)**: (1) `source`는 **대소문자 구분** 문자열로 취급(정규화 없음), (2) `source=`(빈 문자열)·미지 값은 **API 400**, (3) UI는 미지 값 감지 시 **주소줄에서 `source` 키 제거**로 “필터 해제”를 일관되게 수행한다, (4) 허용값 목록은 UI·API가 함께 import하는 단일 상수/타입에 고정(“한 곳”). **검증 힌트(테스트 역할)**: 동일 `tail`(또는 동일 `from`/`to`)에서 `source` 유·무의 `returnedCount/events.length`가 달라짐을 README의 **`curl` 한 줄**로 확인한다.
- [x] **운영·감사 — “현재 뷰 복사”** — `/`에 **현재 적용 중인 `role`·`sessionId`·`from`·`to`·`source`(적용 시)** 를 모두 담은 **절대 URL**을 클립보드로 복사하는 컨트롤(버튼 등)이 있다(적용되지 않은 축은 쿼리 키를 포함하지 않음). **계약(디자인 역할)**: (1) 복사 URL은 화면이 이미 수행한 **정규화 결과**(예: `from`/`to`는 `Z`)를 그대로 담는다, (2) `role`·`sessionId`·`source`는 “현재 UI에 적용 중”인 경우에만 포함(빈 문자열·미지 값은 포함 금지), (3) base는 **현재 origin + pathname(`/`)** 이며 hash는 포함하지 않는다. **검증 힌트(테스트 역할)**: 복사 → 시크릿 창 붙여넣기 후, 주소창 쿼리와 필터 UI가 원본 탭과 동일해야 한다(README에 검증 절차 **한 줄**이 있다).

**성장 루프 확장 (Ralph 사이클 5 · 2026-05-05 테스트 검증 직후)** — URL 재현은 닫혔지만, 현재 홈은 **어떤 필터가 켜졌는지**와 **공유 링크가 실제로 얼마나 쓰이는지**를 운영자가 한눈에 보기 어렵다. *(본질: 공유된 감사 URL이 열렸을 때, 운영자는 같은 뷰를 재현할 뿐 아니라 현재 스코프와 공유 사용량도 즉시 파악할 수 있어야 한다.)*

- **동종(참고)** — [Grafana Explore](https://grafana.com/docs/grafana/latest/explore/)(적용 중 쿼리/시간축이 상단에 계속 드러남), [Sentry Discover](https://docs.sentry.io/product/discover-queries/)(활성 필터를 칩처럼 노출), [Datadog Log Explorer](https://www.datadoghq.com/product/log-management)(공유한 로그 뷰의 사용 맥락을 빠르게 다시 읽게 함). 우리는 재현 URL은 생겼지만 **필터 가시성**과 **공유 사용량 신호**가 아직 약하다.

- [ ] **UI·UX — 적용 중 필터 요약 바** — `/` 타임라인 상단에 현재 적용 중인 `role`·`sessionId`·`source`·`from`·`to`를 **칩/배지 형태**로 한 줄에 보여 주고, 각 칩 또는 전체 초기화로 **원클릭 해제**할 수 있다. 빈 상태에서는 요약 바를 숨기고, 하나라도 적용되면 결과 건수와 함께 드러난다.
- [ ] **운영 — 공유 URL 사용 신호** — 홈에서 “현재 뷰 URL 복사”를 누르거나, `role`·`sessionId`·`source`·`from`·`to` 중 하나가 포함된 URL로 `/`가 열리면 **`application` source 이벤트 또는 동등한 서버 로그**에 `kind`, 적용된 필터 축, 시각이 남는다. README에 공유 링크 복사/열림 신호를 확인하는 절차 한 줄이 있다.
- [ ] **마케팅 — 재현 가능한 감사 링크 사례 추가** — `apps/open-graze/README.md` 또는 랜딩 카피에 “역할·세션·채널·시간축이 고정된 감사 링크” 예시를 짧은 설명과 함께 추가해, 첫 방문자가 제품의 차별점을 스크린샷 없이도 바로 이해할 수 있게 한다.
